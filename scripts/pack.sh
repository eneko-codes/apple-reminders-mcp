...
# Cleared before anything can fail, not just before the zip is written. `set -e` aborts
# this script on a bad signing identity, and a bundle left over from an earlier run then
# sits in dist/ looking current — so the next thing anyone does is install a binary the
# failed run never produced. Observed exactly that: a mistyped MCPB_SIGN_IDENTITY left a
# previous ad-hoc build in place, which would have been installed as though it were the
# signed one.
rm -f "$OUT"
...
otool -P "$BINARY" | grep -q "NSRemindersFullAccessUsageDescription" \
...
otool -P "$STAGE/server/$NAME" | grep -q "NSRemindersFullAccessUsageDescription" \
