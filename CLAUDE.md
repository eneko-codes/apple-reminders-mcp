...
Do not modify, complete, or delete an existing reminder or list. Test reminders in the owner's real store are live data: they need the owner's go-ahead under the rule below, and when allowed must be clearly named `TESTING: ...` and cleaned up when done.
